...
class AIManual(AI):
    ...
    def next_generation(self):
        ...
        print("\tBest 20%%: [%.3f, %.3f]" % (mini, maxi))
        print(old_population)
        print("")
        mini = max(0.0, mini-(random.random()*0.01))
        maxi = min(1.0, maxi+(random.random()*0.01))
        self.population = [random.uniform(mini, maxi) for x in range(self.population_size)]
